...
function contractSymbols(market, asset) {
  if (market === 'mexc_stocks') return [`${asset}STOCK_USDT`, `${asset}_USDT`];
  return [`${asset}_USDT`];
}
...
async function marketTicker(market, asset) {
  ...
  return upstreamFirst(contractSymbols(market, asset).map(symbol => `https://contract.mexc.com/api/v1/contract/ticker?symbol=${encodeURIComponent(symbol)}`));
}
...
async function serverCandles(market, asset, tf) {
  ...
  const data = await upstreamFirst(contractSymbols(market, asset).map(symbol => `https://contract.mexc.com/api/v1/contract/kline/${encodeURIComponent(symbol)}?interval=${encodeURIComponent(futuresTf[tf])}`));
...
async function inspectSignal(asset) {
  ...
  const direction = h4.dir !== 'NEUTRO' && h4.dir === h1.dir && h1.dir === m15.dir ? h4.dir : 'NEUTRO';
  const tradable = direction !== 'NEUTRO' && m15.volRatio >= .8 && m15.r >= 30 && m15.r <= 70 && d1.dir !== (direction === 'LONG' ? 'SHORT' : 'LONG');
  if (!tradable || Date.now() - m15.candle > 60 * 60 * 1000) return;
...
async function monitorSignals() {
  ...
  try { for (const asset of serverWatch.assets) { try { await inspectSignal(asset); } catch (error) { console.warn(`Sinal ${asset}:`, error.message); } } }
  finally { signalScanRunning = false; }
}
...
app.get('/api/health', (_req, res) => {
  ...
  res.json({ ok: true, service: 'SetupDT Light Multi-Exchange V1.8 Auto-Ações', proxy: true, telegramConfigured: Boolean(TELEGRAM_TOKEN && TELEGRAM_CHAT_IDS.length), telegramRecipients: TELEGRAM_CHAT_IDS.length, simulatedTradesOpen: [...activeTrades.values()].filter(row => row.status === 'OPEN').length, monitorSeconds: MONITOR_MS / 1000, signalScanSeconds: SIGNAL_SCAN_MS / 1000, monitoredMarket: serverWatch.market, monitoredAssets: serverWatch.assets.length, binanceFallback: true, oil: 'MEXC UKOIL_USDT + reserva Brent BZ=F', time: new Date().toISOString() });
});

app.get('/api/watchlist', (_req, res) => res.json({ ok: true, ...serverWatch, scanSeconds: SIGNAL_SCAN_MS / 1000 }));
app.post('/api/watchlist', (req, res) => {
  try {
    ...
    serverWatch = { market, assets, updatedAt: Date.now() }; saveWatchlist();
    res.json({ ok: true, ...serverWatch, scanSeconds: SIGNAL_SCAN_MS / 1000 });
    setImmediate(monitorSignals);
  } catch (error) { res.status(400).json({ ok: false, error: String(error.message || error) }); }
});

app.post('/api/telegram/test', async (_req, res) => {
  try { const result = await telegram('✅ SetupDT Light V1.7 conectado. Mensagem de teste para todos os destinatários.'); res.json(result); }
...
app.get('/api/market/klines', async (req, res) => {
  ...
  try {
    ...
    if (p.market === 'binance') {
      ...
      const data = await upstreamFirst(contractSymbols(p.market, p.asset).map(symbol => `https://contract.mexc.com/api/v1/contract/kline/${encodeURIComponent(symbol)}?interval=${encodeURIComponent(futuresTf[p.tf])}`));
      res.set('cache-control', 'no-store');
      return res.json(data);
    }
...
app.listen(PORT, '0.0.0.0', () => console.log(`SetupDT Light V1.8 Auto-Ações online na porta ${PORT}`));
